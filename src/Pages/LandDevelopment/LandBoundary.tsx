...
import { Point, TreeSpacing } from '../../types/land';

function LandBoundary() {
    const [boundary, setBoundary] = useState<Point[]>([]);
    const [scale, setScale] = useState<number>(1); // 1 meter per pixel
    const [spacing, setSpacing] = useState<TreeSpacing>({
      horizontal: 7,
      vertical: 7
    });
    const [showAnalysis, setShowAnalysis] = useState<boolean>(false);
  
    const handleBoundaryUpdate = (points: Point[]) => {
      setBoundary(points);
    };
