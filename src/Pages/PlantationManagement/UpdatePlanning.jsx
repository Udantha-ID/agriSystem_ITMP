...
function UpdatePlanning() {
  ...
  const [formData, setFormData] = useState({
    soilData: { phLevel: 7, texture: '', nutrients: '', quality: 'Medium' },
    fertilizerSchedules: [],
    pestControls: []
  });
  ...
  useEffect(() => {
    const fetchPlanning = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/plannings/${id}`);
        const data = response.data;
        
        const processedData = {
          ...data,
          fertilizerSchedules: data.fertilizerSchedules.map(s => ({
            ...s,
            date: s.date.split('T')[0]
          })),
          pestControls: data.pestControls.map(p => ({
            ...p,
  ...
  }, [formData.soilData.phLevel, formData.soilData.texture, formData.soilData.nutrients]);
  ...
  const addFertilizerSchedule = () => {
    setFormData(prev => ({
      ...
      fertilizerSchedules: [
        ...
        { type: '', date: '', quantity: '', method: 'Broadcast', reminder: false }
      ]
    }));
  };

  const removeFertilizerSchedule = (index) => {
    const newSchedules = formData.fertilizerSchedules.filter((_, i) => i !== index);
  ...
  const validateForm = () => {
    ...
    for (const [index, schedule] of formData.fertilizerSchedules.entries()) {
      for (const field of requiredFertilizerFields) {
        if (!schedule[field]) return `Fertilizer schedule ${index + 1} missing ${field}`;
      }
    }

    for (const [index, control] of formData.pestControls.entries()) {
      for (const field of requiredPestFields) {
        if (!control[field]) return `Pest control ${index + 1} missing ${field}`;
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`http://localhost:5000/plannings/${id}`, {
        ...formData,
      ...
      <div className="p-6 md:pl-64">
        <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-lg overflow-hidden">
          <div className="bg-gradient-to-r from-green-500 to-emerald-600 p-6">
            <h2 className="text-3xl font-bold text-white">Update Planning</h2>
            <p className="text-emerald-100 mt-1">Planning ID: {id}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    <input
                      ...
                      onChange={(e) => handleSoilChange('phLevel', e.target.value)}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => handleSoilChange('texture', e.target.value)}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => handleSoilChange('nutrients', e.target.value)}
                    className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-emerald-500"
                    required
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    className={`w-full p-3 rounded-lg font-medium ${
                      formData.soilData.quality === 'High' ? 'bg-green-100 text-green-800' :
                      formData.soilData.quality === 'Medium' ? 'bg-amber-100 text-amber-800' :
            ...
            <fieldset className="border-2 border-emerald-100 p-6 rounded-lg">
              <legend className="text-lg font-semibold text-emerald-700 px-2">Fertilizer Management</legend>
              
              {formData.fertilizerSchedules.map((schedule, index) => (
                <div key={index} className="space-y-4 mb-6 border-b border-emerald-50 pb-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-600 mb-2">Fertilizer Type *</label>
                      <select
                        value={schedule.type}
                        onChange={(e) => handleFertilizerChange(index, 'type', e.target.value)}
                    ...
                    <div>
                      ...
                      <input
                        type="date"
                        value={schedule.date}
                        onChange={(e) => handleFertilizerChange(index, 'date', e.target.value)}
                        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-emerald-500"
                        required
                      />
                    </div>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-600 mb-2">Quantity (kg/ha) *</label>
                      <input
                        type="number"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-600 mb-2">Application Method</label>
                      <select
                        value={schedule.method}
                        onChange={(e) => handleFertilizerChange(index, 'method', e.target.value)}
                        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-emerald-500"
                      >
                  ...
                  <div className="flex items-center justify-between">
                    <label className="flex items-center gap-2 text-sm text-gray-600">
                      <input
                        ...
                        onChange={(e) => handleFertilizerChange(index, 'reminder', e.target.checked)}
            ...
            <fieldset className="border-2 border-emerald-100 p-6 rounded-lg">
              <legend className="text-lg font-semibold text-emerald-700 px-2">Pest Control Management</legend>

              {formData.pestControls.map((control, index) => (
                <div key={index} className="space-y-4 mb-6 border-b border-emerald-50 pb-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-600 mb-2">Control Method *</label>
                      <select
                        value={control.method}
                        onChange={(e) => handlePestControlChange(index, 'method', e.target.value)}
                        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-emerald-500"
                        required
                      >
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handlePestControlChange(index, 'product', e.target.value)}
                        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-emerald-500"
                    ...
                    <div>
                      ...
                      <input
                        type="date"
                        value={control.date}
                        onChange={(e) => handlePestControlChange(index, 'date', e.target.value)}
                        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-emerald-500"
                        required
                      />
                    </div>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-600 mb-2">Re-application Frequency</label>
                      <select
                        value={control.frequency}
                        onChange={(e) => handlePestControlChange(index, 'frequency', e.target.value)}
                        className="w-full p-2 border rounded-lg focus:ring-2 focus:ring-emerald-500"
                      >
                  ...
                  <div className="flex items-center justify-between">
                    <label className="flex items-center gap-2 text-sm text-gray-600">
                      <input
                        ...
                        onChange={(e) => handlePestControlChange(index, 'reminder', e.target.checked)}
                        className="rounded text-emerald-500 focus:ring-emerald-500"
                      />
